...
async function seed() {
    ...
    try {
        ...
        const senha = await bcryptjs_1.default.hash('Hsa@2026', 12);
        await client.query(`
    ...
    `, [senha]);
        logger_1.logger.info('✅ Usuários criados');
        const adminId = await client.query("SELECT id FROM usuarios WHERE email = 'admin@hsa.com.br'");
        const joaoId = await client.query("SELECT id FROM usuarios WHERE email = 'joao.mendes@hsa.com.br'");
        const carlosId = await client.query("SELECT id FROM usuarios WHERE email = 'carlos.souza@hsa.com.br'");
        const admin = adminId.rows[0]?.id;
        const joao = joaoId.rows[0]?.id;
        const carlos = carlosId.rows[0]?.id;
        if (!admin || !joao || !carlos) {
            logger_1.logger.warn('IDs não encontrados — seed de chamados ignorado');
            return;
        }
        await client.query(`
    ...
    `, [admin, joao, carlos]);
        logger_1.logger.info('✅ Chamados de exemplo criados');
        logger_1.logger.info('');
        logger_1.logger.info('─── Credenciais de acesso ───────────────────');
        logger_1.logger.info('Admin:       admin@hsa.com.br    / Hsa@2026');
        logger_1.logger.info('Gestor:      joao.mendes@hsa.com.br / Hsa@2026');
        logger_1.logger.info('Atendente:   maria.oliveira@hsa.com.br / Hsa@2026');
        logger_1.logger.info('Solicitante: carlos.souza@hsa.com.br / Hsa@2026');
        logger_1.logger.info('─────────────────────────────────────────────');
    }
    catch (error) {
